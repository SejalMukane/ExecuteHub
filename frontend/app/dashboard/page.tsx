...
import { api, BrowserImage, BrowserSession } from "@/lib/api";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [imgRes, sessRes] = await Promise.all([
  ...
  const stats = [
    ...
    { label: "Total Sessions", value: String(sessions.length), icon: Globe },
    {
      label: "Avg. Duration",
      value: sessions.length ? formatDuration(sessions.reduce((sum, s) => sum + s.elapsed, 0) / sessions.length) : "—",
      icon: Clock,
    },
    { label: "Available Browsers", value: String(images.length), icon: Box },
  ];
      ...
      <div className="relative z-10">
          ...
          <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
            <div className="flex items-center gap-8">
              ...
              <nav className="hidden sm:flex items-center gap-6 text-sm">
                ...
                <Link href="/profile" className="text-neutral-400 hover:text-white transition-colors">Profile</Link>
              </nav>
            </div>
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-neutral-400 mt-1">Launch and manage your isolated browser sessions.</p>
          </div>
